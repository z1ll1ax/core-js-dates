...
function isDateInPeriod(date, period) {
  ...
  const dateS = new Date(period.start);
  const dateE = new Date(period.end);
  return dateTarget >= dateS && dateTarget <= dateE;
}
...
function getWeekNumberByDate(date) {
  ...
  let initialDay = (targetDate - startOfYear) / (1000 * 60 * 60 * 24);
  let counter = 1;
  while (initialDay > 7) {
    initialDay -= 7;
    counter += 1;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const firstDate = toDate(period.start);
  ...
  while (iteratorDate <= endDate) {
    for (
      let i = 0;
      i < countWorkDays && iteratorDate <= endDate;
      i += 1, iteratorDate.setDate(iteratorDate.getDate() + 1)
    ) {
      result.push(toStr(iteratorDate));
    }
    iteratorDate.setDate(iteratorDate.getDate() + countOffDays);
  }
  return result;
}
